...
from face_auditor.lib_model.proto_net.model import Convnet, PrototypicalNetwork
from face_auditor.lib_model.few_shot_base import FewShotBase
from face_auditor.parameter_parser import parameter_parser
from face_auditor.lib_dataset.task_dataset import TaskDataset
from face_auditor.lib_dataset.transform import NWays, KShots, LoadData, RemapLabels
...
class ProtoNet(FewShotBase):
    def __init__(self, args):
        ...
        self.model = PrototypicalNetwork(self.args["feature_extractor"], 3, 64, 64).to(
            self.device
        )
    ...
    def convert_layers(
        ...
        for name, module in reversed(model._modules.items()):
            if len(list(module.children())) > 0:
                # Recursion.
                model._modules[name] = self.convert_layers(
    ...
    def train_model(self, train_dset, test_dset, train_num_task: int = None):
        ...
        train_num_task_use = (
            train_num_task
            if train_num_task is not None
            else self.args["train_num_task"]
        )

        for epoch in range(train_num_task_use):
            self.logger.info("epoch: %s" % (epoch,))
            self.model.train()

            for i in range(self.args["train_num_task"]):
                data, labels = next(iter(train_loader))
                _, loss, _ = self.fast_adapt(
                    data,
                    labels,
                    self.args["way"],
                    self.args["shot"],
                ...
                if self.args["is_dp_defense"]:
                    ...
                    self.logger.info(
                        "Epoch: %s | epsilon: %s | best alpha: %s"
                        % (epoch, epsilon, best_alpha)
                    )
    ...
    def evaluate_model(self, test_dset, train_num_task_use: int = None):
        ...
        for i in range(train_num_task_use):
            ...
            _, _, acc = self.fast_adapt(
                data,
                labels,
                self.args["way"],
                self.args["shot"],
                self.args["test_num_query"],
            )
            loss_ctr += 1
            n_acc += acc
    ...
    def probe_model(self, probe_dset):
        ...
        for i, (data, labels, class_label, similarity) in enumerate(probe_loader, 1):
            ...
            logit, _, _ = self.fast_adapt(
                data,
                labels,
                self.args["probe_ways"],
                self.args["probe_shot"],
            ...
            ret_logit.append(
                np.concatenate(
                    logit[: self.args["probe_num_query"]].detach().cpu().numpy()
                )
            )
        ...
        return (
            np.stack(ret_logit),
            torch.stack(train_class).numpy(),
            torch.cat(batch_similarity).numpy(),
        )
    ...
    def _perturbe_scores(self, scores):
        ...
        for i, score in enumerate(scores):
            ret_scores[i] = score + np.random.laplace(
                loc=0.0, scale=self.args["noise_std"], size=score.size
            )
...
if __name__ == "__main__":
    ...
    (
        target_train_dset,
        shadow_train_dset,
        target_train_mem_dset,
        target_train_nonmem_dset,
        shadow_train_mem_dset,
        shadow_train_nonmem_dset,
        target_test_dset,
        shadow_test_dset,
