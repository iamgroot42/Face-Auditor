...
from face_auditor.lib_model.relation_net.model import CNNEncoder, FeatureEncoder, RelationNetwork
...
if __name__ == '__main__':
    ...
    train_transforms = [
        NWays(shadow_train_mem_dset, args['way']),
        KShots(shadow_train_mem_dset, args['train_num_query'] + args['shot']),
        LoadData(shadow_train_mem_dset),
        RemapLabels(shadow_train_mem_dset),
    ]
    test_transforms = [
        NWays(shadow_test_dset, args['way']),
        KShots(shadow_test_dset, args['test_num_query'] + args['shot']),
        LoadData(shadow_test_dset),
        RemapLabels(shadow_test_dset),
    ]

    train_dset = TaskDataset(shadow_train_mem_dset, task_transforms=train_transforms)
    test_dset = TaskDataset(shadow_test_dset, task_transforms=test_transforms,
